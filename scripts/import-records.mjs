...
for(const n of records.filter(r=>r.type==='note')){
 ...
 if(n.request){absolute(n.request.origin);if(typeof n.request.revision!=='string'||!n.request.revision||!['answer','test','correction','reference'].includes(n.contribution_role))throw Error('Invalid request contribution');}
 ...
 const columns=['id','origin','revision','actor_id','author','title','body','topic','kind','tool','version','context','sources','derived_from','request_origin','request_revision','contribution_role','license','basis','state','created_at'];
 const values=[id,n.origin,n.revision,'imported:'+hash(new URL(n.origin).origin+':'+n.actor_id),n.author,n.title,n.body,n.topic||'',n.kind||'note',n.tool||'',n.version||'',JSON.stringify(context),JSON.stringify(n.sources),n.derived_from?JSON.stringify(n.derived_from):null,n.request?.origin||null,n.request?.revision||null,n.contribution_role||'',n.license,n.basis||'Imported contribution','published',n.created_at];
