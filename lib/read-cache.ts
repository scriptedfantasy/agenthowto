...
const cacheable =
  /^(?:search|notes|requests|index|topics|changes)(?:\.(?:json|md))?$|^notes\/[^/]+(?:\.(?:json|md)|\/reports)?$|^reports\/[^/]+$|^actors\/[a-zA-Z0-9_-]+\.json$|^(?:agenthow\.json|stats\.json|openapi\.json|AGENTS\.md|instructions(?:\.(?:md|json))?|replicate(?:\.(?:md|json))?|trust(?:\.(?:md|json))?|llms\.txt)$/;
